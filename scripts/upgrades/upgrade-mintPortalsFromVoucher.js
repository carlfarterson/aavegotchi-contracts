...
async function main() {
  ...
  const newShopFuncs = [
    getSelector("function mintPortalsFromVoucher(uint256 _id, uint256 _amount) external")
  ];
  ...
  const newDaoFuncs = [
    getSelector("function setVoucherContract (address _voucherContract) external")
  ];

  let existingDaoFuncs = getSelectors(daoFacet);
  for (const selector of newDaoFuncs) {
...
exports.mintPortalsFromVoucher = main;
